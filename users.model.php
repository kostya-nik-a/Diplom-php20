...
function updateUser($params = []) {
    ...
    $query = $db->prepare("UPDATE `users` SET `password` = :new_password WHERE `user_id` = :user_id");
...
function getAllUsers($user_id, $sortOfType) {
    ...
    if (isset($sortOfType)) {
        ...
        $query = $db->prepare("SELECT user_id, login, password, email, fio, date_added, user_id_creater FROM users 
        ...
        $query = $db->prepare("SELECT user_id, login, password, email, fio, date_added, user_id_creater FROM users WHERE user_id <> $user_id");
        $query->bindParam(':user_id', $user_id, PDO::FETCH_ASSOC);
        $query->execute();
        return $query->fetchAll();
    }
...
function getWhoCreateUser($user_id_creater) {
    $db = connectDB();
    $query = $db->prepare("SELECT login FROM users WHERE user_id = $user_id_creater");
    $query->execute();
    return $query->fetchAll();
}
...
function getUserById($id) {
    $db = connectDB();
    $query = $db->prepare("SELECT user_id, login, fio FROM users WHERE user_id = :user_id");
...
function getUserByLogin($login) {
    $db = connectDB();
    $sql = "SELECT user_id, login, password FROM users WHERE `login` = :login";
    $query = $db->prepare($sql);
